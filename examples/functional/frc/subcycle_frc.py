...
if __name__ == '__main__':
    ...
    # Time parameters
    nr_save_steps = 50
    simulation_time = 2e-7
    save_frequency = simulation_time / nr_save_steps
    ramp_time = simulation_time / 40.0
    courant_factor = 0.4
    subcycling_factor = 1
    em_dt = (1.0 / (c * np.sqrt(3.0 / (dx ** 2.0))))
    hydro_dt = subcycling_factor * em_dt
    ...
    with tqdm(total=simulation_time, desc="Simulation Progress") as pbar:
        while current_time < simulation_time:
            ...
            #print(f"Time Step Hydro: {hydro_dt}")
            ...
            # Subcycle the EM fields
            for i in range(subcycling_factor):
            ...
            if (remander + hydro_dt) > save_frequency:
                save_index += 1
                ...
                print(f"Total Charge Density: {np.sum(charge_density.data.numpy())}")
                #if save_index == 10:
                #    exit()
                # Print total mass
            ...
            if (remander + hydro_dt) > 2.0 * save_frequency:
                wp.synchronize()
                toc = time.time()
            ...
            # Update the time
            current_time += hydro_dt

            # Update the progress bar
            pbar.update(hydro_dt)
