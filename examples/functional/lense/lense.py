...
if __name__ == '__main__':
    ...
    origin = (-5.0, -5.0, -5.0)
    spacing = (dx, dx, dx)
    shape = (int(10.0 / dx), int(10.0 / dx), int(10.0 / dx))
    ...
    start_x = -3.2 # x location of the wave source
    frequency = (1.0 * c) / (2.0 * 3.14159 * 0.25) # Resonant frequency of the sphere
    wavelength = c / frequency
    amplitude = 1.0
    sigma = 0.3333333333 * wavelength
    ...
    eps_mapping = wp.from_numpy(np.array([eps, lense_eps], dtype=np.float32), dtype=wp.float32)
    mu_mapping = wp.from_numpy(np.array([mu, lense_mu], dtype=np.float32), dtype=wp.float32)
    sigma_e_mapping = wp.from_numpy(np.array([sigma_e, lense_sigma_e], dtype=np.float32), dtype=wp.float32)
    sigma_m_mapping = wp.from_numpy(np.array([sigma_m, lense_sigma_m], dtype=np.float32), dtype=wp.float32)
    ...
    for step in tqdm(range(num_steps)):
        ...
        if step % save_frequency == 0:
            field_saver([id_field, electric_field], os.path.join(output_dir, f"electric_field_{str(step).zfill(4)}.vtk"))
