...
class IonEulerUpdate(Operator):
    ...
    def _apply_fluxes(
        ...
        if id_stencil[0] == wp.uint8(0):
            ...
            # Add emission if wall
            if id_stencil[1] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, emission_factor * dt * spacing[1] * spacing[2] * flux_face_x, field_e.shape, c, i, j, k
                )
            if id_stencil[2] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, emission_factor * dt * spacing[0] * spacing[2] * flux_face_y, field_e.shape, c, i, j, k
                )
            if id_stencil[3] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, emission_factor * dt * spacing[0] * spacing[1] * flux_face_z, field_e.shape, c, i, j, k
                )
        ...
        if id_stencil[1] == wp.uint8(0):
            ...
            if id_stencil[0] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, -emission_factor * dt * spacing[1] * spacing[2] * flux_face_x, field_e.shape, c, i + 1, j, k
                )
        if id_stencil[2] == wp.uint8(0):
            periodic_atomic_add(
            ...
            if id_stencil[0] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, -emission_factor * dt * spacing[0] * spacing[2] * flux_face_y, field_e.shape, c, i, j + 1, k
                )
        if id_stencil[3] == wp.uint8(0):
            periodic_atomic_add(
            ...
            if id_stencil[0] != wp.uint8(0):
                periodic_atomic_add(
                    field_e.data, -emission_factor * dt * spacing[0] * spacing[1] * flux_face_z, field_e.shape, c, i, j, k + 1
                )
    ...
    def __init__(
        ...
        # Set emission factor
        emission_factor = wp.constant(wp.float32(emission_factor))
        ...
        def _euler_update(
            ...
            energy: Fieldfloat32,
            mass_e: Fieldfloat32,
            momentum_e: Fieldfloat32,
            energy_e: Fieldfloat32,
            id_field: Fielduint8,
            gamma: wp.float32,
            dt: wp.float32,
        ):
            ...
            for c in range(4):
                ...
                rho, vx, vy, vz, p = IonEulerUpdate._extrapolate_half_time_step(
                    rho_p4_stencil[c],
            ...
            (
                ...
                flux_mom_z_face_x,
                flux_energy_face_x,
            ) = IonEulerUpdate._compute_fluxes(
                rho_faces[1],
                rho_faces[0],
                vx_faces[1],
                vx_faces[0],
