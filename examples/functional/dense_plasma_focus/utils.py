...
class PlasmaInitializer(Operator):
    ...
    def _initialize(
        ...
        # Get x, y, z
        x = wp.float32(id_field_i) * density_i.spacing[0] + id_field.origin[0]
        y = wp.float32(id_field_j) * density_i.spacing[1] + id_field.origin[1]
        z = wp.float32(id_field_k) * density_i.spacing[2] + id_field.origin[2]

        # inner and outer radius bounds
        inner_radius = 0.004
        outer_radius = 0.0115
        r = wp.sqrt(x**2.0 + y**2.0)

        # Set density
        density_i.data[0, i, j, k] = plasma_number_density * proton_mass
        density_e.data[0, i, j, k] = plasma_number_density * electron_mass
        #if z > 0.0076 and z < 0.0200 and r < outer_radius and r > inner_radius:
        #    density_i.data[0, i, j, k] = plasma_number_density * proton_mass
        #    density_e.data[0, i, j, k] = plasma_number_density * electron_mass
        #else:
        #    density_i.data[0, i, j, k] = 0.001 * plasma_number_density * proton_mass
        #    density_e.data[0, i, j, k] = 0.001 * plasma_number_density * electron_mass
