...
if __name__ == "__main__":
    ...
    dx = 0.0003 # 1 mm
    origin = (-0.06, -0.020, -0.016) # meters
    spacing = (dx, dx, dx)
    shape = (int(0.12 / dx), int(0.040 / dx), int(0.032 / dx))
    ...
    # Chamber parameters
    # Geometry
    chamber_wall_thickness = 0.0036
    interaction_radius = 0.003
    interaction_bounds = 0.01
    acceleration_bounds = 0.03
    formation_radius = 0.007
    formation_bounds = 0.04
    diverter_inlet_radius = 0.003
    diverter_inlet_bounds = 0.045
    diverter_radius = 0.007
    diverter_bounds = 0.05
    background_id = 1
    vacuum_id = 0
    chamber_id = 2
    # Material
    ...
    # Geometry
    insulator_thickness = 0.0008
    dielectric_thickness = 0.0006
    insulator_id = 3
    conductor_id = 4
    # Material
    ...
    interaction_cable_thickness_r = 0.0006
    interaction_cable_thickness_y = 0.0010
    interaction_dielectric_id = [i for i in range(5, 21)]
    interaction_switch_id = [i for i in range(21, 37)]
    interaction_resistor_id = 37
    # Material
    interaction_voltage = 1000 # V
    interaction_capacitance = 1.0 # F
    interation_switch_start_time = 1.5e-7 # s
    interaction_switch_time = 1e-8 # s
    interaction_switch_end_time = 4.0e-7 # s
    for i, dielectric_id in enumerate(interaction_dielectric_id):
        initial_e_mapping[dielectric_id] = (1 - 2 * (i % 2)) * interaction_voltage / dielectric_thickness
    ...
    accelerator_cable_thickness_r = 0.0006
    accelerator_cable_thickness_y = 0.0010
    accelerator_dielectric_id = [i for i in range(38, 46)]
    accelerator_switch_id = [i for i in range(46, 54)]
    accelerator_resistor_id = 54
    # Material
    accelerator_voltage = 1000 # V
    accelerator_capacitance = 1.0 # F
    accelerator_switch_start_time = 7.5e-8 # s
    accelerator_switch_time = 1e-8 # s
    accelerator_switch_end_time = 4.0e-7 # s
    for i, dielectric_id in enumerate(accelerator_dielectric_id):
        initial_e_mapping[dielectric_id] = (1 - 2 * (i % 2)) * accelerator_voltage / dielectric_thickness
        eps_mapping[dielectric_id] = capacitance_to_eps(
    ...
    formation_cable_thickness_r = 0.0006
    formation_cable_thickness_y = 0.0010
    formation_dielectric_id = [i for i in range(55, 63)]
    formation_switch_id = [i for i in range(63, 71)]
    formation_resistor_id = 71
    # Material
    formation_voltage = 1000 # V
    formation_capacitance = 1.0 # F
    formation_switch_start_time = 0.0 # s
    formation_switch_time = 1.0e-8 # s
    formation_switch_end_time = 4.0e-7 # s
    for i, dielectric_id in enumerate(formation_dielectric_id):
        initial_e_mapping[dielectric_id] = (1 - 2 * (i % 2)) * formation_voltage / dielectric_thickness
        eps_mapping[dielectric_id] = capacitance_to_eps(
    ...
    diverter_inlet_cable_thickness_r = 0.0006
    diverter_inlet_cable_thickness_y = 0.0004
    diverter_inlet_dielectric_id = [i for i in range(72, 74)]
    diverter_inlet_switch_id = [i for i in range(74, 76)]
    diverter_inlet_resistor_id = 76
    # Material
    diverter_inlet_voltage = 2000 # V
    diverter_inlet_capacitance = 1.0 # F
    diverter_inlet_switch_start_time = 0.0 # s
    diverter_inlet_switch_time = 1.0e-8 # s
    diverter_inlet_switch_end_time = 4.0e-7 # s
    for i, dielectric_id in enumerate(diverter_inlet_dielectric_id):
        initial_e_mapping[dielectric_id] = (1 - 2 * (i % 2)) * diverter_inlet_voltage / dielectric_thickness
        eps_mapping[dielectric_id] = capacitance_to_eps(
    ...
    # Time parameters
    ramp_time = 1e-10  # 10 nanoseconds
    relaxation_time = 1.0e-9  # 10 nanoseconds
    simulation_time = 4e-7  # 1 microsecond
    save_interval = 5e-10  # 10 nanoseconds
