...
class Chamber(Compound):
    ...
    def __init__(
        ...
        chamber = Location((0, 0, -chamber_thickness)) * Circle(
            chamber_diameter / 2 + chamber_thickness
        )
        chamber = extrude(chamber, amount=chamber_height + 2 * chamber_thickness)

        # Subtract the feedthroughs
        feedthrough = Location((0, 0, -chamber_thickness)) * Circle(
            feedthrough_diameter / 2
        )
