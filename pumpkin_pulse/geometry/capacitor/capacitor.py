...
class Capacitor(Compound):
    ...
    def __init__(
        ...
        cable_conductor_0 = Location((0, 0, -conductor_plate_thickness)) * Circle(
            cable_diameter / 2
        )
        cable_conductor_0 = loft(
            [cable_conductor_0, Rectangle(capacitor_width, capacitor_width)]
        )
        ...
        # Create dielectric
        dielectric = Location((0, 0, conductor_plate_thickness)) * Rectangle(
        ...
        conductor_plate_1 = Location(
            (0, 0, conductor_plate_thickness + dielectric_thickness)
        ) * Rectangle(capacitor_width, capacitor_width)
        ...
        cable_conductor_1 = Location(
            (0, 0, 3 * conductor_plate_thickness + dielectric_thickness)
        ) * Circle(cable_diameter / 2)
        cable_conductor_1 = loft(
            [
                cable_conductor_1,
                Location((0, 0, 2 * conductor_plate_thickness + dielectric_thickness))
                * Rectangle(capacitor_width, capacitor_width),
            ]
        )
        ...
        # Create insulator
        insulator = Location((0, 0, -conductor_plate_thickness)) * Rectangle(
            capacitor_width + 2.0 * cable_insulator_thickness,
            capacitor_width + 2.0 * cable_insulator_thickness,
        )
        insulator = extrude(
            insulator, amount=4 * conductor_plate_thickness + dielectric_thickness
        )
        insulator = insulator - (
            conductor_plate_0
            + cable_conductor_0
        ...
        RigidJoint(
            ...
            to_part=self,
            joint_location=Location(
                (0, 0, -1 * conductor_plate_thickness), (0, 0, 180)
            ),
        )
