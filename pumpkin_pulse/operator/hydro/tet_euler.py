...
class EulerUpdate(Operator):
    ...
    def __init__(
        self,
        inside_domain = None,
        apply_boundary_conditions = None
    ):
        # Set functions
        if inside_domain is None:
            inside_domain = self._always_inside_domain
        ...
        @wp.kernel
        def _euler_update(
            centers: wp.array(dtype=wp.vec2f),
            edges: wp.array(dtype=wp.vec2f),
            center_to_edge: wp.array2d(dtype=wp.int32),
            density: wp.array(dtype=wp.float32),
            velocity: wp.array(dtype=wp.vec2f),
            pressure: wp.array(dtype=wp.float32),
            mass: wp.array(dtype=wp.float32),
            momentum: wp.array(dtype=wp.vec2f),
            ...
            rho_stencil, rho_stencil_dxyz, vx_stencil, vx_stencil_dxyz, vy_stencil, vy_stencil_dxyz, vz_stencil, vz_stencil_dxyz, p_stencil, p_stencil_dxyz = self.apply_boundary_conditions(
                i, j, k, density.origin, density.spacing, density.shape, rho_stencil, rho_stencil_dxyz, vx_stencil, vx_stencil_dxyz, vy_stencil, vy_stencil_dxyz, vz_stencil, vz_stencil_dxyz, p_stencil, p_stencil_dxyz, gamma, dt
            )
            ...
            # Allocate fluxes
            total_flux_mass = wp.float32(0.0)
            total_flux_mom_x = wp.float32(0.0)
            total_flux_mom_y = wp.float32(0.0)
            total_flux_mom_z = wp.float32(0.0)
            total_flux_energy = wp.float32(0.0)
            ...
            for face in range(6):
                ...
                # Update fluxes
                total_flux_mass += flux_mass
                total_flux_mom_x += flux_mom_x
                total_flux_mom_y += flux_mom_y
                total_flux_mom_z += flux_mom_z
                total_flux_energy += flux_energy

            # Update variables
            mass.data[0, i, j, k] += total_flux_mass
            momentum.data[0, i, j, k] += total_flux_mom_x
            momentum.data[1, i, j, k] += total_flux_mom_y
            momentum.data[2, i, j, k] += total_flux_mom_z
            energy.data[0, i, j, k] += total_flux_energy
