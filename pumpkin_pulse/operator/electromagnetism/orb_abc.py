...
class OutgoingRadiationBoundaryCondition:
    """
    ...
    def __init__(
        ...
        def _update_electric_field(
            ...
            ## Break on edge
            #if i == electric_field.shape[0] - 1 and j == electric_field.shape[1] - 1:
            #    return
            #if i == electric_field.shape[0] - 1 and k == electric_field.shape[2] - 1:
            #    return
            #if j == electric_field.shape[1] - 1 and k == electric_field.shape[2] - 1:
            #    return
            ...
            c = 299792458.0
            coef = (c * dt - electric_field.spacing[0]) / (c * dt + electric_field.spacing[0])
            if old_i == 0 or old_i == electric_field.shape[0] - 1:
                #new_ijk_e[0] = old_ijk_e[0]
                new_ijk_e[1] = (
                    electric_field.data[1, i, j, k]
                    + coef * (new_ijk_e[1] - electric_field.data[1, old_i, j, k])
                )
                new_ijk_e[2] = (
                    electric_field.data[2, i, j, k]
                    + coef * (new_ijk_e[2] - electric_field.data[2, old_i, j, k])
                )
            elif old_j == 0 or old_j == electric_field.shape[1] - 1:
                new_ijk_e[0] = (
                    electric_field.data[0, i, j, k]
                    + coef * (new_ijk_e[0] - electric_field.data[0, i, old_j, k])
                )
                #new_ijk_e[1] = old_ijk_e[1]
                new_ijk_e[2] = (
                    electric_field.data[2, i, j, k]
                    + coef * (new_ijk_e[2] - electric_field.data[2, i, old_j, k])
                )
            elif old_k == 0 or old_k == electric_field.shape[2] - 1:
                new_ijk_e[0] = (
                    electric_field.data[0, i, j, k]
                    + coef * (new_ijk_e[0] - electric_field.data[0, i, j, old_k])
                )
                new_ijk_e[1] = (
                    electric_field.data[1, i, j, k]
                    + coef * (new_ijk_e[1] - electric_field.data[1, i, j, old_k])
                )
                #new_ijk_e[2] = old_ijk_e[2]
            if old_i == 0:
                new_ijk_e[0] = old_ijk_e[0]
            if old_j == 0:
                new_ijk_e[1] = old_ijk_e[1]
            ...
            # Set electric field
            #if old_i == electric_field.shape[0] - 1:
            #    electric_field.data[1, i, old_j, k] = new_ijk_e[1]
            #    electric_field.data[2, i, old_j, k] = new_ijk_e[2]
            #if old_j == electric_field.shape[1] - 1:
            #    electric_field.data[0, old_i, j, k] = new_ijk_e[0]
            #    electric_field.data[2, old_i, j, k] = new_ijk_e[2]
            #if old_k == electric_field.shape[2] - 1:
            #    electric_field.data[0, old_i, j, k] = new_ijk_e[0]
            #    electric_field.data[1, old_i, j, k] = new_ijk_e[1]
            #else:
            electric_field.data[0, old_i, old_j, old_k] = new_ijk_e[0]
            electric_field.data[1, old_i, old_j, old_k] = new_ijk_e[1]
            electric_field.data[2, old_i, old_j, old_k] = new_ijk_e[2]
...
class YeeMagneticFieldUpdate(Operator):
    ...
    def __init__(
        ...
        def _update_magnetic_field(
            ...
            # Set magnetic field
            if i != magnetic_field.shape[0] - 1:
                magnetic_field.data[0, i, j, k] = new_h[0]
            if j == magnetic_field.shape[1] - 1:
                magnetic_field.data[1, i, j, k] = new_h[1]
            if k == magnetic_field.shape[2] - 1:
