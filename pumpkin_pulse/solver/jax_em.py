...
def set_box(material, eps, mu, sigma_e, sigma_m, color, origin, size):
    """
...
def get_material_properties_xyz(eps, mu, sigma_e, sigma_m):
    # Get material properties on staggered grid
    # eps
    eps_x = (
        eps
        + jnp.roll(eps, 1, axis=1)
        + jnp.roll(eps, 1, axis=2)
        + jnp.roll(jnp.roll(eps, 1, axis=1), 1, axis=2)
    ) / 4.0
    eps_y = (
        eps
        + jnp.roll(eps, 1, axis=0)
        + jnp.roll(eps, 1, axis=2)
        + jnp.roll(jnp.roll(eps, 1, axis=0), 1, axis=2)
    ) / 4.0
    eps_z = (
        eps
        + jnp.roll(eps, 1, axis=0)
        + jnp.roll(eps, 1, axis=1)
        + jnp.roll(jnp.roll(eps, 1, axis=0), 1, axis=1)
    ) / 4.0
    ...
    sigma_e_x = (
        sigma_e
        + jnp.roll(sigma_e, 1, axis=1)
        + jnp.roll(sigma_e, 1, axis=2)
        + jnp.roll(jnp.roll(sigma_e, 1, axis=1), 1, axis=2)
    ) / 4.0
    sigma_e_y = (
        sigma_e
        + jnp.roll(sigma_e, 1, axis=0)
        + jnp.roll(sigma_e, 1, axis=2)
        + jnp.roll(jnp.roll(sigma_e, 1, axis=0), 1, axis=2)
    ) / 4.0
    sigma_e_z = (
        sigma_e
        + jnp.roll(sigma_e, 1, axis=0)
        + jnp.roll(sigma_e, 1, axis=1)
        + jnp.roll(jnp.roll(sigma_e, 1, axis=0), 1, axis=1)
    ) / 4.0

    # sigma_m
    sigma_m_x = (2.0 * sigma_m * jnp.roll(sigma_m, 1, axis=0)) / (
        sigma_m + jnp.roll(sigma_m, 1, axis=0) + 1e-10
    )
    sigma_m_y = (2.0 * sigma_m * jnp.roll(sigma_m, 1, axis=1)) / (
        sigma_m + jnp.roll(sigma_m, 1, axis=1) + 1e-10
    )
    sigma_m_z = (2.0 * sigma_m * jnp.roll(sigma_m, 1, axis=2)) / (
        sigma_m + jnp.roll(sigma_m, 1, axis=2) + 1e-10
    )
...
def fix_e_y(electric_field, e0, time, tau, origin, size):
    """
    ...
    # Fix electric field
    alpha = (4.0 / tau) ** 2
    electric_field = electric_field.at[x0 : x1 + 1, y0:y1, z0 : z1 + 1, 1].set(
        e0 * jnp.exp(-alpha * (time - tau) ** 2)
    )
...
def update_switch_conductivity(
    sigma_e, time, tau_start, tau_end, sigma_switch, origin, size
):
    """
    ...
    sigma_e = sigma_e.at[x0:x1, y0:y1, z0:z1].set(
        sigma_switch * jnp.exp(-alpha * (time - tau_end) ** 2)
    )
...
def update_electric_field(
    ...
    return jnp.concatenate(
        [new_electric_field_x, new_electric_field_y, new_electric_field_z], axis=-1
    )


@partial(jax.jit, donate_argnums=(1))
def update_magnetic_field(
    electric_field, magnetic_field, impressed_magnetic_current, mu, sigma_m, dt, dx
):
    # Get coefficients
    ...
    return jnp.concatenate(
        [new_magnetic_field_x, new_magnetic_field_y, new_magnetic_field_z], axis=-1
    )
...
if __name__ == "__main__":
    ...
    eps = (
        jnp.ones((size * scale_factor, size * scale_factor, size * scale_factor))
        * Vacuum.eps
    )
    mu = (
        jnp.ones((size * scale_factor, size * scale_factor, size * scale_factor))
    ...
    color = (
        jnp.zeros((size * scale_factor, size * scale_factor, size * scale_factor))
        * Vacuum.color
    )
    ...
    electric_field = jnp.zeros(
        (size * scale_factor, size * scale_factor, size * scale_factor, 3)
    )
    magnetic_field = jnp.zeros(
        (size * scale_factor, size * scale_factor, size * scale_factor, 3)
    )

    # Initialize impressed current
    impressed_electric_current = jnp.zeros(
        (size * scale_factor, size * scale_factor, size * scale_factor, 3)
    )
    impressed_magnetic_current = jnp.zeros(
        (size * scale_factor, size * scale_factor, size * scale_factor, 3)
    )
    ...
    for t in tqdm(range(nr_steps)):
        ...
        if t % plot_freq == 0:
            ...
            im1 = axes[0, 0].imshow(
                electric_field[:, :, scale_factor * size // 2, 1], cmap="viridis"
            )
            ...
            im2 = axes[0, 2].imshow(
                magnetic_field[:, :, scale_factor * size // 2, 2], cmap="plasma"
            )
            ...
            # Color plot
            im3 = axes[1, 2].imshow(
                color[:, :, scale_factor * size // 2], cmap="inferno"
            )
            ...
            if time < tau:
                plt.suptitle(
                    f"Charging capacitor at t (us) = {time / 1e-6:.3f}", fontsize=32
                )
            elif time < tau_switch_start:
                plt.suptitle(f"Steady State at t (us) = {time / 1e-6:.4f}", fontsize=32)
            else:
                plt.suptitle(
                    f"Discharging capacitor at t (us) = {time / 1e-6:.3f}", fontsize=32
                )
