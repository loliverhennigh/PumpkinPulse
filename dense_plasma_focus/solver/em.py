...
def construct_em_update_kernels():
    ...
    def _update_eletric_field(
        ...
        curl_h_x = (
            (magnetic_field_z[i, j, k] - magnetic_field_z[i, j - 1, k])
          - (magnetic_field_y[i, j, k] - magnetic_field_y[i, j, k - 1])
        )
        curl_h_y = (
            (magnetic_field_x[i, j, k] - magnetic_field_x[i, j, k - 1])
          - (magnetic_field_z[i, j, k] - magnetic_field_z[i - 1, j, k])
        )
        curl_h_z = (
            (magnetic_field_y[i, j, k] - magnetic_field_y[i - 1, j, k])
        ...
        e = wp.vec3f(
            electric_field_x[i, j, k], electric_field_y[i, j, k], electric_field_z[i, j, k]
        )
        cur = wp.vec3f(
    ...
    def _update_magnetic_field(
        ...
        curl_e_x = (
            (electric_field_y[i, j, k+1] - electric_field_y[i, j, k])
          - (electric_field_z[i, j+1, k] - electric_field_z[i, j, k])
        )
        curl_e_y = (
            (electric_field_z[i+1, j, k] - electric_field_z[i, j, k])
          - (electric_field_x[i, j, k+1] - electric_field_x[i, j, k])
        )
        curl_e_z = (
            (electric_field_x[i, j+1, k] - electric_field_x[i, j, k])
        ...
        h = wp.vec3f(
            magnetic_field_x[i, j, k], magnetic_field_y[i, j, k], magnetic_field_z[i, j, k]
        )
        new_h = (
...
if __name__ == "__main__":
    ...
    material_id = voxelize_compound(
        compound=reactor,
        origin=(origin[0] - dx/2.0, origin[1] - dx/2.0, origin[2] - dx/2.0), # shift origin by half a voxel to center on ghost cell
        resolution=dx,
        nr_voxels=nr_voxels,
        materials=materials,
    )
    ...
    # Get em kernels
    update_electric_field_kernel, update_magnetic_field_kernel = construct_em_update_kernels()
    ...
    print("time:", toc - tic)
    print("million voxels per second:", nr_steps * np.prod(internal_voxels) / (toc - tic) / 1e6)
