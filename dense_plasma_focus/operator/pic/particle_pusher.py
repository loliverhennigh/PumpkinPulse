...
class BorisVelocityUpdate(Operator):
    ...
    def _feild_to_particle(
        ...
        f_1_1_1 = feild[feild_index, ijk_lower[0] + 1, ijk_lower[1] + 1, ijk_lower[2] + 1]

        # Get position for each dimension
        cell_pos_x_lower = feild_origin[0] + wp.float32(ijk_lower[0]) * feild_spacing[0] + feild_spacing[0] / 2.0
        cell_pos_y_lower = feild_origin[1] + wp.float32(ijk_lower[1]) * feild_spacing[1] + feild_spacing[1] / 2.0
        cell_pos_z_lower = feild_origin[2] + wp.float32(ijk_lower[2]) * feild_spacing[2] + feild_spacing[2] / 2.0
    ...
    def _get_electric_field(
        ...
        origin_ex = origin - wp.vec3f(0.0, spacing[1]/2.0, spacing[2]/2.0) - ghost_cell_offset
        origin_ey = origin - wp.vec3f(spacing[0]/2.0, 0.0, spacing[2]/2.0) - ghost_cell_offset
        origin_ez = origin - wp.vec3f(spacing[0]/2.0, spacing[1]/2.0, 0.0) - ghost_cell_offset

        # Get lower cell index for E components
        f_ijk_ex = wp.cw_div(pos - origin_ex, spacing) - wp.vec3f(0.5, 0.5, 0.5) # -0.5 to get the lower cell index
        ...
        ijk_ex = wp.vec3i(wp.int32(f_ijk_ex[0]), wp.int32(f_ijk_ex[1]), wp.int32(f_ijk_ex[2]))
        ijk_ey = wp.vec3i(wp.int32(f_ijk_ey[0]), wp.int32(f_ijk_ey[1]), wp.int32(f_ijk_ey[2]))
        ijk_ez = wp.vec3i(wp.int32(f_ijk_ez[0]), wp.int32(f_ijk_ez[1]), wp.int32(f_ijk_ez[2]))

        # Get electric field
        e_x = BorisVelocityUpdate._feild_to_particle(pos, ijk_ex, electric_field, 0, origin_ex, spacing)
        e_y = BorisVelocityUpdate._feild_to_particle(pos, ijk_ey, electric_field, 1, origin_ey, spacing)
        e_z = BorisVelocityUpdate._feild_to_particle(pos, ijk_ez, electric_field, 2, origin_ez, spacing)
    ...
    def _get_magnetic_field(
        ...
        f_ijk_bx = wp.cw_div(pos - origin_bx, spacing) - wp.vec3f(0.5, 0.5, 0.5) # -0.5 to get the lower cell index
        ...
        ijk_bx = wp.vec3i(wp.int32(f_ijk_bx[0]), wp.int32(f_ijk_bx[1]), wp.int32(f_ijk_bx[2]))
        ijk_by = wp.vec3i(wp.int32(f_ijk_by[0]), wp.int32(f_ijk_by[1]), wp.int32(f_ijk_by[2]))
        ijk_bz = wp.vec3i(wp.int32(f_ijk_bz[0]), wp.int32(f_ijk_bz[1]), wp.int32(f_ijk_bz[2]))

        # Get magnetic field
        b_x = BorisVelocityUpdate._feild_to_particle(pos, ijk_bx, magnetic_field, 0, origin_bx, spacing)
        b_y = BorisVelocityUpdate._feild_to_particle(pos, ijk_by, magnetic_field, 1, origin_by, spacing)
        b_z = BorisVelocityUpdate._feild_to_particle(pos, ijk_bz, magnetic_field, 2, origin_bz, spacing)
    ...
    def _update_velocity(
        ...
        pos = wp.vec3f(particle_position[0, i], particle_position[1, i], particle_position[2, i])
        vel = wp.vec3f(particle_velocity[0, i], particle_velocity[1, i], particle_velocity[2, i])
        ...
        e = BorisVelocityUpdate._get_electric_field(pos, electric_field, origin, spacing, nr_ghost_cells)
        b = BorisVelocityUpdate._get_magnetic_field(pos, magnetic_field, origin, spacing, nr_ghost_cells)
...
class PositionUpdate(Operator):
    ...
    def _move_particle(
        ...
        ijk_material = wp.vec3i(wp.int32(f_ijk_material[0]), wp.int32(f_ijk_material[1]), wp.int32(f_ijk_material[2]))
        ...
        # Check if particle in a boundary cell
        if material_id[wp.int32(ijk_material[0]), wp.int32(ijk_material[1]), wp.int32(ijk_material[2])] != 0:
            particle_id[0, particle_index] = wp.uint8(0) # Kill particle
    ...
    def _current_deposition(
        ...
        current_density_origin = origin - wp.vec3f(spacing[0]/2.0, spacing[1]/2.0, spacing[2]/2.0) - ghost_cell_offset
        ...
        for i in range(5):
            for j in range(5):
                for k in range(5):
                    ...
                    # Calculate current deposition vector
                    w_x = PositionUpdate._w_helper_function(s_old[i, 0], s_old[j, 1], s_old[k, 2], s_new[i, 0], s_new[j, 1], s_new[k, 2])
                    w_y = PositionUpdate._w_helper_function(s_old[j, 1], s_old[i, 0], s_old[k, 2], s_new[j, 1], s_new[i, 0], s_new[k, 2])
                    w_z = PositionUpdate._w_helper_function(s_old[k, 2], s_old[j, 1], s_old[i, 0], s_new[k, 2], s_new[j, 1], s_new[i, 0])
                    ...
                    if i != 0:
                        current_mat[i * 25 + j * 5 + k, 0] += current_mat[(i - 1) * 25 + j * 5 + k, 0]
                    if j != 0:
                        current_mat[i * 25 + j * 5 + k, 1] += current_mat[i * 25 + (j - 1) * 5 + k, 1]
                    if k != 0:
                        current_mat[i * 25 + j * 5 + k, 2] += current_mat[i * 25 + j * 5 + (k - 1), 2]

        # Add current matrix to global current density
        for i in range(5):
            for j in range(5):
                for k in range(5):
                    wp.atomic_add(current_density, 0, wp.int32(ijk_rho[0]) + i - 2, wp.int32(ijk_rho[1]) + j - 2, wp.int32(ijk_rho[2]) + k - 2, current_mat[i * 25 + j * 5 + k, 0])
                    wp.atomic_add(current_density, 1, wp.int32(ijk_rho[0]) + i - 2, wp.int32(ijk_rho[1]) + j - 2, wp.int32(ijk_rho[2]) + k - 2, current_mat[i * 25 + j * 5 + k, 1])
                    wp.atomic_add(current_density, 2, wp.int32(ijk_rho[0]) + i - 2, wp.int32(ijk_rho[1]) + j - 2, wp.int32(ijk_rho[2]) + k - 2, current_mat[i * 25 + j * 5 + k, 2])
    ...
    def _update_position(
        ...
        # Get new position
        new_pos = PositionUpdate._move_particle(i, pos, vel, particle_id, material_id, origin, spacing, dt, nr_ghost_cells)

        # If particle is out of bounds, kill it
        if new_pos[0] < origin[0] - 2.0 * spacing[1] or new_pos[0] > origin[0] + spacing[0] * wp.float32(current_density.shape[1] - 4 * nr_ghost_cells):
            particle_id[0, i] = wp.uint8(0)
            return
        if new_pos[1] < origin[1] - 2.0 * spacing[1] or new_pos[1] > origin[1] + spacing[1] * wp.float32(current_density.shape[2] - 4 * nr_ghost_cells):
            particle_id[0, i] = wp.uint8(0)
            return
        if new_pos[2] < origin[2] - 2.0 * spacing[1] or new_pos[2] > origin[2] + spacing[2] * wp.float32(current_density.shape[3] - 4 * nr_ghost_cells):
            particle_id[0, i] = wp.uint8(0)
            return


        # Deposit current density
        PositionUpdate._current_deposition(pos, new_pos, mass, charge, current_density, origin, spacing, dt, nr_ghost_cells)
...
class DepositCharge(Operator):
    ...
    def _deposit_charge(
        ...
        # Deposit charge
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]), wp.int32(ijk_rho[1]), wp.int32(ijk_rho[2]), charge * s_x_lower * s_y_lower * s_z_lower)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]) + 1, wp.int32(ijk_rho[1]), wp.int32(ijk_rho[2]), charge * s_x_upper * s_y_lower * s_z_lower)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]), wp.int32(ijk_rho[1]) + 1, wp.int32(ijk_rho[2]), charge * s_x_lower * s_y_upper * s_z_lower)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]) + 1, wp.int32(ijk_rho[1]) + 1, wp.int32(ijk_rho[2]), charge * s_x_upper * s_y_upper * s_z_lower)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]), wp.int32(ijk_rho[1]), wp.int32(ijk_rho[2]) + 1, charge * s_x_lower * s_y_lower * s_z_upper)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]) + 1, wp.int32(ijk_rho[1]), wp.int32(ijk_rho[2]) + 1, charge * s_x_upper * s_y_lower * s_z_upper)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]), wp.int32(ijk_rho[1]) + 1, wp.int32(ijk_rho[2]) + 1, charge * s_x_lower * s_y_upper * s_z_upper)
        wp.atomic_add(charge_density, 0, wp.int32(ijk_rho[0]) + 1, wp.int32(ijk_rho[1]) + 1, wp.int32(ijk_rho[2]) + 1, charge * s_x_upper * s_y_upper * s_z_upper)
