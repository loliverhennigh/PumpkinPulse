# This file contains the base class for the particle in cell solver.
...
from pumpkin_pulse.plasma import Plasma
from pumpkin_pulse.operator.voxelize.build123d import Build123DVoxelize
from pumpkin_pulse.operator.electromagnetism.electromagnetism import (
    ElectricFieldUpdate,
    MagneticFieldUpdate,
    SetElectricField,
    ElectricFieldToChargeDensity,
    ComputeElectricEnergy,
    ComputeMagneticEnergy,
    SumChargeDensity,
)
from pumpkin_pulse.operator.pic.particle_pusher import (
    BorisVelocityUpdate,
    PositionUpdate,
    DepositCharge,
    InitializeParticles,
    ComputeParticleEnergy,
)


class EMPICSolver:
    """
    Base Solver class for particle in cell simulations.
    """
    ...
    def __init__(
        ...
        particle_buffer_size: int = int(10e7),
    ):
        ...
        self.origin = origin
        self.shape = shape
        self.nr_ghost_cells = 1
        self.nr_solids = len(solids)
        self.shape_with_ghost_cells = [
            shape[i] + 2 * self.nr_ghost_cells for i in range(3)
        ]
        self.compute_statistics_frequency = compute_statistics_frequency
        self.save_fields_frequency = save_fields_frequency
        self.particle_buffer_size = particle_buffer_size
        ...
        self.solid_id = wp.zeros((1, *self.shape_with_ghost_cells), dtype=wp.uint8)

        # Make electromagnetic arrays
        self.electric_field = wp.zeros((3, *self.shape_with_ghost_cells), wp.float32)
        self.magnetic_field = wp.zeros((3, *self.shape_with_ghost_cells), wp.float32)
        self.impressed_current = wp.zeros((3, *self.shape_with_ghost_cells), wp.float32)
        self.charge_density = wp.zeros((1, *self.shape_with_ghost_cells), wp.float32)

        # Make solid property mappings
        self.eps_mapping = wp.zeros(self.nr_solids + 1, wp.float32)
        self.mu_mapping = wp.zeros(self.nr_solids + 1, wp.float32)
        self.sigma_e_mapping = wp.zeros(self.nr_solids + 1, wp.float32)
        self.sigma_m_mapping = wp.zeros(self.nr_solids + 1, wp.float32)
        self.permiable_mapping = wp.zeros(self.nr_solids + 1, wp.uint8)

        # Make arrays for storing surface and volume charge densities
        self.total_surface_charge = wp.zeros(self.nr_solids + 1, wp.float32)
        self.total_volume_charge = wp.zeros(self.nr_solids + 1, wp.float32)

        # Make arrays for storing plasmas particles
        self.nr_particle_types = int(np.sum([len(plasma.particles) for plasma in plasmas]) + 1) # 0 is killed particles
        self.particle_position = wp.zeros((3, self.particle_buffer_size), wp.float32)
        self.particle_velocity = wp.zeros((3, self.particle_buffer_size), wp.float32)
        self.particle_id = wp.zeros((1, self.particle_buffer_size), wp.uint8)
        self.particle_mass_mapping = wp.zeros(self.nr_particle_types, wp.float32)
        self.particle_charge_mapping = wp.zeros(self.nr_particle_types, wp.float32)
        self.nr_particles = 0

        # Make operators
        self.voxelizer = Build123DVoxelize()
        self.electric_field_update = ElectricFieldUpdate()
        self.magnetic_field_update = MagneticFieldUpdate()
        self.set_electric_field = SetElectricField()
        self.electric_field_to_charge_density = ElectricFieldToChargeDensity()
        self.compute_electric_energy = ComputeElectricEnergy()
        self.compute_magnetic_energy = ComputeMagneticEnergy()
        self.sum_charge_density = SumChargeDensity()
        self.boris_velocity_update = BorisVelocityUpdate()
        self.position_update = PositionUpdate()
        self.deposit_charge = DepositCharge()
        self.initialize_particles = InitializeParticles()
        self.compute_particle_energy = ComputeParticleEnergy()
    ...
    def update_material_properties(self):
        ...
        if self.time == 0.0 or any(
            ...
            # Create np mappings
            eps_mapping = np.zeros(self.nr_solids + 1, np.float32)
            mu_mapping = np.zeros(self.nr_solids + 1, np.float32)
            sigma_e_mapping = np.zeros(self.nr_solids + 1, np.float32)
            sigma_m_mapping = np.zeros(self.nr_solids + 1, np.float32)
            permiable_mapping = np.zeros(self.nr_solids + 1, np.uint8)

            # Set vacuum properties
            eps_mapping[0] = VACUUM.eps
            mu_mapping[0] = VACUUM.mu
            sigma_e_mapping[0] = VACUUM.sigma_e
            sigma_m_mapping[0] = VACUUM.sigma_m
            permiable_mapping[0] = np.uint8(0)

            for i, solid in enumerate(self.solids):
                material = solid.material
                eps_mapping[i + 1] = material.eps
                mu_mapping[i + 1] = material.mu
                if material.constant_conductivity:
                    sigma_e_mapping[i + 1] = material.sigma_e
                else:
                    sigma_e_mapping[i + 1] = material.sigma_e(self.time)
                sigma_m_mapping[i + 1] = material.sigma_m
                permiable_mapping[i + 1] = np.uint8(solid.permiable)
            ...
            self.permiable_mapping = wp.from_numpy(permiable_mapping, wp.uint8)

    def update_plasma_properties(self):
        """
        Update the plasma properties in the solver.
        """

        # Check if we need to update properties
        particle_mass_mapping = np.zeros(self.nr_particle_types, np.float32)
        particle_charge_mapping = np.zeros(self.nr_particle_types, np.float32)

        # Set vacuum properties
        particle_mass_mapping[0] = 1.0 # Killed particles mass to avoid division by zero
        particle_charge_mapping[0] = 0.0 # Killed particles charge to avoid division by zero

        index = 1
        for i, plasma in enumerate(self.plasmas):
            for j, particle in enumerate(plasma.particles):
                particle_mass_mapping[index] = particle.mass * plasma.micro_to_macro_ratio / (self.spacing[0] * self.spacing[1] * self.spacing[2])
                particle_charge_mapping[index] = particle.charge * plasma.micro_to_macro_ratio / (self.spacing[0] * self.spacing[1] * self.spacing[2])
                index += 1

        # Update properties
        self.particle_mass_mapping = wp.from_numpy(particle_mass_mapping, wp.float32)
        self.particle_charge_mapping = wp.from_numpy(particle_charge_mapping, wp.float32)

    def initialize_solid_id(self):
        """
        Initialize the solid id array.
        """
        print("Initializing solid id...")

        # Voxelize solids
        for i, solid in enumerate(self.solids):
            self.solid_id = self.voxelizer(
                self.solid_id,
                solid.geometry,
                i + 1,
                self.spacing,
    ...
    def initialize_plasma(self):
        ...
        for i, plasma in enumerate(self.plasmas):
            ...
            # Get particle ids to add
            add_species_ids = np.zeros(len(plasma.particles), np.uint32)
            for j in range(len(plasma.particles)):
                add_species_ids[j] = index
                index += 1

            # Set particle mass and charge
            self.particle_position, self.particle_velocity, self.particle_id, self.nr_particles = self.initialize_particles(
                self.particle_position,
                self.particle_velocity,
                self.particle_id,
                self.particle_mass_mapping,
                self.nr_particles,
                plasma.nr_macro_particles,
                add_species_ids,
                plasma.geometry,
                plasma.temperature,
            )
    ...
    def initialize_electromagnetic_fields(self):
        ...
        initialization_steps = 400 * int(np.max(self.shape))
